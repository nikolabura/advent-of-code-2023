...
input =
  ...
  |> Enum.map(fn [og_hand, bid_str] ->
    bid = String.to_integer(bid_str)
    hand = String.replace(og_hand, "JJJJJ", "KKKKK")

    freqs = String.graphemes(hand)
      |> Enum.frequencies()

    {most_common, _} = freqs
      |> Enum.reject(fn {k, _} -> k == "J" end)
      |> Enum.max_by(fn {_, v} -> v end)
    replaced_hand = String.replace(hand, "J", most_common)
    IO.puts("#{hand} -> #{replaced_hand}")

    freqs = String.graphemes(replaced_hand)
      |> Enum.frequencies()
      |> Map.values()
      |> Enum.sort()
      |> Enum.reverse()
    ...
    IO.puts("Hand #{replaced_hand} is #{type}.")
  ...
  |> Enum.sort(fn {greater_hand, greater_type, _}, {lesser_hand, lesser_type, _} ->
    ...
    if type_score.(greater_type) != type_score.(lesser_type) do
      ...
      results = Enum.map(zipped, fn {greater, lesser} ->
        order = String.graphemes("J23456789TQKA")
